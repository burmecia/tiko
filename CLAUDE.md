...
- **Sync path**: calls `core::relfork_ops` (read/write blocks) directly in the backend process. Correct
...
- **Async path** (`tiko_startreadv` → `aio.rs::perform_io`): uses the shared-memory pipeline to
  `tikoworker`. Falls back to direct `core::relfork_ops` calls when the worker/pipeline is unavailable
...
(or falls back to direct `core::relfork_ops` calls when the pipeline isn't available) and waits on the
...
`core::relfork_ops` calls when the async path isn't available — initdb, shutdown checkpoint, worker crash.
Sync smgr functions always call `core::relfork_ops` directly regardless, so pages land in the shmem cache /
