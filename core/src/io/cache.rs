...
use super::io_control::IoControl;
use crate::{
...
impl CacheControl {
    ...
    fn evict(&self) -> u32 {
        for _ in 0..(self.num_slots * MAX_USAGE_COUNT as u32) {
            ...
            if dirty != 0 {
                ...
                super::io_control::IoControl::get()
                    .stats
                    .dirty_evictions
    ...
    /// Flush dirty blocks from a cache slot to Store express.
    ///
    /// 1. PUTs the full 256 KB chunk to the express-bucket `latest` object via
    ///    `Store::put_express_latest` — a plain PUT, no staging, no
    ...
    ///
    /// Both steps are guarded: they are skipped when `Store` or `ProjectCtx`
    ...
    pub fn flush_dirty_chunk(&self, slot_index: u32) {
        ...
        // Guard: only run when Store and ProjectCtx are initialised.
        if let (Some(sim), Some(ctx)) = (
            crate::store::Store::try_get(),
